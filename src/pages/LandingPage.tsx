  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden mesh-bg">
        ...
        <div className="max-w-4xl mx-auto px-5 sm:px-6 pt-32 pb-40 text-center relative">
          <div className="inline-flex items-center gap-2 chip animate-fade-up mb-8" style={{ background: 'rgb(var(--primary) / .08)', borderColor: 'rgb(var(--primary) / .2)' }}>
            ...
            <span style={{ color: 'rgb(var(--primary))' }}>Professional Portfolio Builder</span>
          </div>
          <h1 className="font-display text-[3.25rem] sm:text-6xl md:text-[5.5rem] font-semibold leading-[1.02] tracking-tight animate-fade-up delay-1">
            Build a Portfolio<br />
            That <span className="gradient-text">Opens Doors</span>
          </h1>
          <p className="mt-9 text-lg md:text-xl max-w-2xl mx-auto leading-relaxed animate-fade-up delay-2" style={{ color: 'rgb(var(--text-soft))' }}>
            Create a professional portfolio in minutes. Showcase your projects, skills, and achievements with confidence.
          </p>
          <div className="mt-12 flex flex-col sm:flex-row gap-3.5 justify-center animate-fade-up delay-3">
            <Link to="/signup" className="btn btn-primary btn-lg">Get Started <ArrowRight size={18} /></Link>
            <a href="#features" className="btn btn-outline-cyan btn-lg">Explore Features</a>
          </div>
        </div>
      </section>

      {/* Features */}
      <section id="features" className="max-w-6xl mx-auto px-5 sm:px-6 py-24">
      ...
      <section id="how" className="py-24" style={{ background: 'rgb(var(--bg-soft))' }}>
        <div className="max-w-6xl mx-auto px-5 sm:px-6">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {steps.map((s, i) => (
              <div key={s.n} className={`card card-hover p-8 animate-fade-up delay-${i + 1}`}>
      ...
      {/* Showcase / CTA */}
      <section id="showcase" className="max-w-6xl mx-auto px-5 sm:px-6 py-24 text-center">
        <div className="card glass p-12 md:p-20 shadow-xl relative overflow-hidden">
          ...
          <h2 className="font-display text-3xl md:text-4xl font-semibold relative">Ready to build yours?</h2>
      ...
      <footer className="border-t" style={{ borderColor: 'rgb(var(--border))' }}>
        <div className="max-w-6xl mx-auto px-5 sm:px-6 py-10 flex flex-col md:flex-row items-center justify-between gap-4">
          <Logo size="sm" />
